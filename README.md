...
## Kustom Skrip Deterministik (Folder `scripts`)

Definisi kustom skrip di dalam folder `scripts` (ketika LLM dinonaktifkan melalui `manual_only: true` atau `disable_llm: true`) adalah metode otomatisasi deterministik yang mengeksekusi file JavaScript secara langsung pada proses bot untuk menjalankan rangkaian aksi yang kompleks. Mekanisme ini bekerja dengan konsep yang mirip dengan `allow_insecure_coding` (yaitu mengeksekusi kode JavaScript mentah secara langsung untuk mengontrol bot), namun perbedaannya terletak pada aspek keamanan dan prediktabilitas; alih-alih meminta LLM membuat kode baru secara dinamis di runtime yang berisiko mengalami kesalahan logika atau halusinasi, metode ini menjalankan kode statis yang sudah ditulis dan divalidasi terlebih dahulu oleh pengguna di komputer lokal. Begitu perintah `!runScript` dipicu, bot akan mengimpor file tersebut secara dinamis dan memberikan akses penuh ke API dasar bot (`bot`), pustaka aksi (`skills`), serta deteksi lingkungan (`world`), memungkinkan bot melakukan tugas-tugas terintegrasi secara otomatis, cepat, dan 100% offline tanpa ketergantungan pada API Model Bahasa (LLM).

### Panduan Membuat Kustom Skrip

Agar kustom skrip dapat berjalan dengan baik dan stabil, ikuti petunjuk berikut:

1. **Format File**: Simpan skrip JavaScript Anda di dalam folder `scripts/` di direktori utama proyek dengan ekstensi `.js`.
2. **Ekspor Utama (Export)**: Skrip Anda harus mengekspor fungsi default (`export default function`) atau fungsi `main` (`export function main`) yang bertipe async. Fungsi ini akan dipanggil oleh bot dengan tiga parameter utama:
   ```javascript
   export default async function main(bot, skills, world) {
       // Kode Anda di sini
   }
   ```
3. **Menggunakan Aksi Terintegrasi (`skills`)**: Selalu gunakan fungsi-fungsi asinkron dari `skills` dengan `await` untuk memastikan aksi berjalan secara berurutan dan bot menunggu setiap aksi selesai (misalnya: `await skills.goToPlayer(bot, "PlayerName")` atau `await skills.breakBlockAt(bot, x, y, z)`).
4. **Keamanan & Penanganan Kesalahan**: Bungkus kode Anda menggunakan blok `try...catch` untuk mengantisipasi kesalahan saat eksekusi agar proses bot tidak terhenti secara mendadak jika terjadi kegagalan (misalnya jika bot kehabisan durabilitas alat atau jalurnya terhalang).
5. **Cara Menjalankan**: Kirimkan pesan obrolan di Minecraft/LAN dengan perintah `!runScript("nama_file_anda")` atau `!runScript("nama_file_anda.js")`.

Contoh skrip sederhana dapat Anda lihat langsung di [scripts/example_script.js](file:///d:/Games/Hades/MC21_11/scripts/example_script.js).
