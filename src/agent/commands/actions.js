...
export const actionsList = [
    ...
    {
        name: '!putAllChest',
        description: 'Put all items from the inventory into the nearest chest, except for tools and food.',
        params: { },
        perform: runAsAction(async (agent) => {
            await skills.putAllInChest(agent.bot);
        })
    },
    {
        name: '!viewChest',
    ...
    {
        name: '!runScript',
        description: 'Run a custom JavaScript automation script from the scripts folder.',
        params: {
            'script_name': { type: 'string', description: 'The filename of the script to run (without the path, e.g. "test_script.js").' }
        },
        perform: runAsAction(async (agent, script_name) => {
            const path = await import('path');
            const { pathToFileURL } = await import('url');
            let scriptPath = script_name;
            if (!scriptPath.endsWith('.js')) {
                scriptPath += '.js';
            }
            const fullPath = path.resolve('./scripts', scriptPath);
            try {
                const moduleUrl = pathToFileURL(fullPath).href;
                const scriptModule = await import(moduleUrl);
                const runFunc = scriptModule.default || scriptModule.main;
                if (typeof runFunc !== 'function') {
                    throw new Error('Script does not export a default function or main function.');
                }
                const skills = await import('../library/skills.js');
                const world = await import('../library/world.js');
                await runFunc(agent.bot, skills, world);
                console.log(`Successfully executed script ${script_name}`);
            } catch (err) {
                console.error(`Error running script ${script_name}:`, err);
                agent.bot.chat(`Error running script ${script_name}: ${err.message}`);
            }
        })
    }
];
